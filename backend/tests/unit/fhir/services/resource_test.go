...
func TestExtractSubjectRef(t *testing.T) {
	t.Run("valid subject", func(t *testing.T) {
		...
		ref := services.ExtractSubjectRef(data)
...
func TestExtractPatientFieldRef(t *testing.T) {
	t.Run("valid patient", func(t *testing.T) {
		...
		ref := services.ExtractPatientFieldRef(data)
...
func TestExtractStatusField(t *testing.T) {
	t.Run("has status", func(t *testing.T) {
		...
		status := services.ExtractStatusField(data)
		if status != "active" {
...
func TestGenericSearchBundle(t *testing.T) {
	t.Run("empty results", func(t *testing.T) {
		bundle := services.GenericSearchBundle(nil, 0, "Observation", 20, 0, "http://localhost")
		if bundle["total"] != 0 {
	...
	t.Run("with results", func(t *testing.T) {
		...
		bundle := services.GenericSearchBundle(resources, 1, "Observation", 20, 0, "http://localhost")
		if bundle["total"] != 1 {
	...
	t.Run("pagination next link", func(t *testing.T) {
		bundle := services.GenericSearchBundle(nil, 50, "Encounter", 20, 0, "http://localhost")
	...
	t.Run("pagination previous link", func(t *testing.T) {
		bundle := services.GenericSearchBundle(nil, 50, "Encounter", 20, 20, "http://localhost")
		links := bundle["link"].([]map[string]string)
		hasPrev := false
		for _, l := range links {
	...
	t.Run("no next link on last page", func(t *testing.T) {
		bundle := services.GenericSearchBundle(nil, 10, "Patient", 20, 0, "http://localhost")
		links := bundle["link"].([]map[string]string)
		for _, l := range links {
			if l["relation"] == "next" {
