...
func setupRouter(handler *handlers.PatientHandler) *gin.Engine {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.Use(auth.AuthTestMiddleware())
...
func TestCreatePatient_201(t *testing.T) {
	...
	handler := handlers.NewPatientHandler(mock)
...
func TestCreatePatient_400_InvalidJSON(t *testing.T) {
	handler := handlers.NewPatientHandler(&mockPatientService{})
	router := setupRouter(handler)
